...
Deno.serve(async (req) => {
  ...
  try {
    ...
    // ── منع الكتب الرقمية عبر هذا المسار (COD/غير-Paymob) ─────────────────
    // هذه الدالة ترفض Paymob بالفعل (أعلاه)، فأي طلب يمر من هنا هو دفع عند
    // الاستلام أو ما شابه — لا معنى له لملف رقمي (لا يوجد تسليم فعلي يُحصَّل
    // عنده الثمن)، وقد يمنح وصولاً مجانيًا لو أكّد الأدمن الطلب لاحقًا كإجراء
    // روتيني. is_digital هنا محسوبة من variant_type في قاعدة البيانات، لا من
    // بيانات العميل، فلا يمكن تجاوز هذا الفحص بالتلاعب بالطلب.
    if (verifiedItems.some((i) => i.is_digital)) {
      return jsonError('الكتب الرقمية تتطلب الدفع الإلكتروني (Paymob) — لا يتوفر الدفع عند الاستلام لها.');
    }
